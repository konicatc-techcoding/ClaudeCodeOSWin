...
日期：2026-07-23（v2；v1 同日稍早）　狀態：**v2 已核准並完工——2026-07-23 使用者
拍板（§10 五項均採建議預設，含 §3.2 殘餘風險知情確認）；已於 P2 完成後實作
（commit `0bbd6c1`，master），2026-07-23～24 經使用者驗收（實測打字輸入與
中文回覆通過）**
...
- **完工紀錄（2026-07-24）**：**已實作完成並經使用者驗收**——commit
  `0bbd6c1`（master，遷移案 P3；依排序拍板於 P2 `3da90ae` 完成後實作）。
  驗收方式：使用者於瀏覽器內實測打字輸入與中文回覆，通過；另含按鈕樣式
  與 15px 字體的目視回饋調整。驗收判準對照 §8 DoD 九條；殘餘風險邊界
  （§3.2）與生命週期拍板值（§5）依核准內容原樣落地。DoD 勾稽摘要另見
  遷移案 §10.2。
...
## 8. Definition of Done（P3 驗收判準）— ✅ 已隨 commit `0bbd6c1` 達成並經使用者驗收（見版本標記完工紀錄）
...
| TUI 相容性問題（ConPTY×xterm.js×claude） | 體驗劣化/不可用 | P3 開工實測（DoD 6）；不可用時誠實回報再議，不硬上。**完工實證**：使用者實測打字與中文回覆通過 |
