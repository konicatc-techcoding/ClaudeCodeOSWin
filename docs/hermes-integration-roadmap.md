...
## Stage 0.5 — 平台收尾清單（部分完成；殘項建議 Stage 2 前清完）
...
這些是 Stage 0 排查留下的小項。殘項不阻塞 Stage 2 前置作業的開始，但**建議在 Stage 2
開工前清完**——gateway 健康狀態直接影響 session 資料是否持續累積，而 Stage 2 的自動化
管線以「session 有在產生」為前提。
...
| financialresearch 自啟腳本修復（腳本已壞） | ⏳ 殘項 | engineering |
| codereviewer profile 去留（從未裝自啟） | ⏳ 殘項（使用者決定） | 使用者；移除則 automation 執行 |
| 三個殭屍 Startup vbs 清除 | ⏳ 殘項 | automation |
| sticky profile 緩解（自動化一律 `--profile default` 的習慣／機制化） | ⏳ 殘項 | engineering／automation |
...
**原 Stage 1 DoD 1／2（真實 session 實走 `to-inbox` → `consolidate-memory` → 正本＋
idempotent 驗證）未執行、不計入完成**——移列為 Stage 2 開工前的必要前置（見下）。
「先跑過人工路徑，自動化才不是在編碼猜測」的原始理由不變，只是位置移動。

---

## Stage 2 — Session Bridge 自動化（cron 偵測 → export → enqueue → headless CoS）

> **進度更新（2026-07-09）**：設計問題 2（去重狀態）的**記錄格式**已先行定稿——見
> [memory-bridge-state.md](memory-bridge-state.md) 與
> [`registry/bridge_state_schema.yaml`](../registry/bridge_state_schema.yaml)
> （`claudecodeos.bridge_state.v1`）；儲存載體仍是實作前的 Stage 2 決策。
> 另外 capability → 執行通道的對應已有 registry 層定義
> （[capability-lanes.md](capability-lanes.md)），bridge state 的
> `selected_capability_lane` 欄位引用其 lane id。bridge 本身仍未實作。
...
- **必要前置（gate，2026-07-09 依 Stage 1 checkpoint 更新）**——全部完成才開工：
  1. **原 Stage 1 DoD 1／2 實走**：至少一個真實（非測試）Hermes session 經
     `to-inbox` → `consolidate-memory` 進入正本；重複執行同一 session 的 `to-inbox`
     驗證為 idempotent（`open(mode="x")` 拒絕覆寫 + `event_id` 去重）；全程零寫入
     Hermes 原始資料。負責：`knowledge`。
  2. **前置決策三項**（使用者／CoS 拍板）：bridge 跑哪一側（見設計問題 1）、
     bridge state 儲存載體（schema 已定稿）、`model_router.yaml` TODO 佔位值確認。
  3. **建議同時清完 Stage 0.5 殘項**（見上表）；並依 checkpoint 第 6 節建議，
     在 bridge 動工前完成 Windows 側 `git init`（無版控是目前 rollback 能力的最大缺口）。
- **負責領域**：`engineering`（bridge 實作、去重狀態、測試）；`automation`
  （排程與工作流串接）；`knowledge`（前置實走與下游整併，沿用既有機制）。
- **必須先決的設計問題**（實作前決定，不留給實作當下即興）：
  1. **Bridge 跑在哪一側**。兩個約束方向相反：(a) Stage 0 實測證明 Windows Hermes
     運行中時 WSL 的一般 SQLite 存取（含唯讀）被互斥擋下——WSL 側 bridge **必須**走
     adapter 的 snapshot／`immutable=1` 路徑；(b) **job pipeline 原生 Windows 跑不動**
     （Stage 1 確立的架構事實：worker 靠 POSIX script + systemd，`jobs.db` 與 worker
     都在 WSL 側）——Windows 側 bridge 的 `enqueue()` 反而要跨界寫 WSL 側 jobs.db。
     取捨是「WSL 側＋snapshot 讀」（與既有服務同構、enqueue 本地）vs「Windows 側直讀
     ＋跨界 enqueue」；就既有約束看，WSL 側＋snapshot 是阻力較小的一案，但正式定案
     留給前置決策。
  2. **去重狀態**：格式已定稿（`claudecodeos.bridge_state.v1`），載體未定；
     不可寫進 Hermes 資料。
  3. **雜訊控制**：測試性質 session（類比 `_hermes_bridge_test` 的健康檢查訊號）不應
     每次都進 CoS 深究——政策即 memory-taxonomy 4.2／4.3。
- **Definition of Done**：
  ...
  3. 零寫入 Hermes 原始資料；bridge 自身狀態存放於 ClaudeCodeOS 側。
...
## 建議優先順序（總結，2026-07-09 更新）

1. ~~Stage 1~~ ✅ 完成（Pre-Bridge Foundation，見 [stage1-checkpoint.md](stage1-checkpoint.md)）。
2. **Stage 2 必要前置**（現在的第一優先）：原 Stage 1 DoD 1／2 實走（knowledge）→
   前置決策三項拍板 → Stage 0.5 殘項與 `git init`。
3. **Stage 2**：前置全過才開工。
4. **Stage 3**：觀測性收尾。
...
| WSL 互斥限制 vs 自動化 | WSL 側排程任務在 Windows Hermes 運行時無法一般讀取 state.db | Stage 2 設計問題 1 明確先決：snapshot/immutable 路徑或 Windows 側執行；DoD 1 直接驗證 |
...
| gateway 自啟不齊（Stage 0.5 殘項未清） | session 資料累積不完整，Stage 2 價值打折 | Stage 0.5 殘項建議在 Stage 2 前清完 |
| **Windows 開發正本無版控**（2026-07-09 新增） | 程式層 rollback 只能靠 WSL pre-sync tarball（僅部署側）；bridge 這種長期演化元件無變更歷史 | Stage 2 前置建議完成 `git init`（sync plan 已列為 v0.2 升級路徑）；rollback 現況索引見 checkpoint 第 6 節 |
