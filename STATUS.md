...
- **★★ `consolidate-memory` 在 headless 排程下結構性做不到(09-04 發現,批次 4 議程)**:
  ...
  6.5 分、**$0.98**),但實際回報是「**分析完成,但無法落盤(權限阻斷)**」——
  `knowledge` 的 `Write`/`Edit` 在 headless session 被拒,而 **consolidate-memory
  本質上就是要寫 `memory/*.md` 正本**,與 CLAUDE.md「headless 不得編輯正本」的邊界
  直接互斥。inbox 未搬動,**下次排程會重新偵測到同樣的檔**。
  **這條 cron 會每天成功、每天產出完整方案、每天落不了盤,而且回報 `completed`
  所以什麼都不會浮出來**——與 `[SILENT]` 那條是同一個形狀(`completed` ≠ 做對了事),
  新的新鮮度燈號也抓不到(它判的是「有沒有在跑」,這條確實在跑)。
  批次 0 那次是**手動**派 knowledge 才清掉的,現在知道那不是巧合、是唯一可行路徑。
...
- **rss 死信率 41% 貼著門檻(09-04 新增)**:`33/80`,離 `dead_letter_ratio_threshold`
  ...
  (例如 0.3)或改看趨勢而非二分門檻;未動 registry。
